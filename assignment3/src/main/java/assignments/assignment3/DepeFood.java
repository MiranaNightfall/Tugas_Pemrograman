...
public class DepeFood {
    ...
    public static void handleBayarBill(String orderId, String paymentOption) {
        ...
        System.out.printf("Berhasil Membayar Bill sebesar Rp %s dengan biaya transaksi sebesar Rp %s\n", decimalFormat.format(order.getTotalHarga()), decimalFormat.format(amountToPay - order.getTotalHarga()));
    }
    ...
    public static void setPenggunaLoggedIn(User user) {
        userLoggedIn = user;
        for (User u : userList) {
            if (u.getNama().equals(user.getNama()) && u.getNomorTelepon().equals(user.getNomorTelepon())) {
                u.setSaldo(user.getSaldo()); // Update the saldo of the corresponding user in the userList
                break;
            }
        }
    }
